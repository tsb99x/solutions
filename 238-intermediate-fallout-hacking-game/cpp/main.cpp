...
range diff_map[] = {{4, 5}, {6, 8}, {9, 10}, {11, 12}, {13, 15}};

int get_rand_from_range(const range &range, default_random_engine &dre) {
  uniform_int_distribution range_dist(range.from, range.to);
  return range_dist(dre);
}

vector<string> filter_words(const vector<string> &dict, const size_t length,
...
vector<string> select_words(const vector<string> &words, const int count) {
  vector<string> res;
...
int guess(const int guesses_left, const string &word) {
...
void try_guess(const string &word, int guesses_left) {
